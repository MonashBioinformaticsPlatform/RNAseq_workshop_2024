...
If few samples have a count of more than around 10, we can safely discard them. We can't require reads in more samples than our group size, or we won't be able to detect genes expressed only in one group. Here our group size is 3, so we will require at least 3 samples with a non-trivial count. So that we treat each sample in the same way, we will set a threshold on "Counts Per Million" (CPM) rather than the raw counts. We have a library size of around 20 million in each sample so we will require a CPM of 0.5.
...
### Filtering rule of thumb {- .rationale}

Our rule of thumb is to require a CPM corresponding to a count of around 10 in a typical sample, in as many samples as our smallest group.

**Discussion:** You could try relaxing either of these settings to see if more differentially expressed genes are found, especially if you have many samples in each group.
...
* Residual variance is calculated from all groups, not just the groups being compared, to improve statistical power (much like post-hoc comparisons in an ANOVA).
...
* Smaller counts are are noisier after log transformation. The "voom" method takes this into account using "precision weights". This is acts as an adjustment to the residual variance of each observation.
...
Multi-Dimensional Scaling (MDS) provides a 2D layout of your samples, attempting to preserve "distances" between the samples. MDS is very similar to Principal Components Analysis (PCA). In fact in Degust it is exactly PCA. **TODO: Check**
