...
> Might need a different figure here - want to keep this figure but might be better in a different section? e.g under sequencing protocols
...
The most common type of analysis performed with RNAseq data is differential gene expression (DGE) analysis. This analysis looks at the difference in abundance for genes between conditions. However it is not the only type of differential test that can be performed with RNAseq data.
...
> Finally have an exercise here that builds on the previous sections to get participants to design their own experiment based on what they are interested in
