...
RNAseq data can be used for a variety of purposes. The most common one is quantative analysis of both gene and transcript expression changes. It can also be used for discovery, such as detection of novel transcripts, fusion genes. In organisms without a reference genome, RNAseq data can be used for de novo transcriptome assembly. 
...
| Biological material                  | RNA targets                                   | Read length                           | RNA isolation method  |
|--------------------------------------|-----------------------------------------------|---------------------------------------|---------------------|
| Entire tissue: Bulk RNAseq           | All RNA types captured: total RNAseq          | 50-300 bases: short read sequencing   | PolyA pulldown      |
| Individual cells: Single cell RNAseq | Specific RNA subset captured: targeted RNAseq | 10-100kb: long read sequencing        | Ribosomal depletion |
|                                      |                                               | 100-300kb: ultra long read sequencing | Short RNA selection |
...
Short read sequencing typically refers to protocols that fragment RNA into smaller pieces before converting to cDNA and sequencing. These are typically highly accurate and cost effective methods that scale well for large datasets. This is typically used for gene expression studies.
...
When a reference genome is either unavailable or not desired, it is possible to take the RNAseq reads and assemble them into a transcriptome of the protein-coding genes. The assembly can be done in 2 ways, there are reference based methods and de novo methods. Reference based methods use the reference of either the organism or a closely related species. 
...
Long read sequencing is better suited to building high quality transcriptomes than short read sequencing as the longer read length retains more information and context about the original RNA. A commonly used analogy when describing assembly is to imagine the genome as a puzzle that needs to be put back together. The smaller the pieces, the more difficult the puzzle. A puzzle made of larger pieces is much easier to put back together. 
