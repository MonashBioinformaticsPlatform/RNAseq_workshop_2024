...
RNAseq data can be used for a variety of purposes. The most common one is quantative analysis of gene expression changes, though isoform level differential analysis can also be performed. RNAseq can also be used for discovery, such as detection of novel transcripts or fusion genes. In organisms without a reference genome, RNAseq data can be used for de novo transcriptome assembly. 
...
These categories of RNAseq are not mutually exclusive. For example, most RNAseq experiments tend to be short read bulk RNAseq that target polyadenylated RNA.

| Biological material                  | RNA targets                                   | RNA isolation method | Read length                           |
|--------------------------------------|-----------------------------------------------|----------------------|---------------------------------------|
| Entire tissue: Bulk RNAseq           | All RNA types captured: total RNAseq          | Ribosomal depletion  | 50-300 bases: short read sequencing   |
| Individual cells: Single cell RNAseq | Specific RNA subset captured: targeted RNAseq | PolyA pulldown       | 10-100kb: long read sequencing        |
|                                      |                                               | Short RNA selection  | 100-300kb: ultra long read sequencing |
...
- Ribosomal depletion: this method removes rRNA, leaving the mRNA, precursor messenger RNA (pre-mRNA) and non-coding RNAs. 

- PolyA pulldown: this targets only RNA with a polyA tail - enriching for mRNAs only. 


### Targeted RNAseq


Advantages: requires less RNA input. 
...
There are trade-offs between bulk and single cell sequencing. The dominant single cell sequencing technology, 10x, detects the most abundant genes and mostly captures polyadenylated mRNA at the 3' end or the 5' end. 
...
```{r, echo=FALSE, fig.align="center", fig.cap ="In a DGE analysis, this gene might be ignored for having no change in expression when actually a change is occuring amongst the isoforms that are being produced"}
