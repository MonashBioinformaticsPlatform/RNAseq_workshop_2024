...
![](images/pipeline/simple_pipeline_counting.svg){width="100%" fig-alt="Simple RNA-seq pipeline overview: Counting / abundance estimation step"}
...
It provides summarization and quantification of mapped reads.

```
aligned reads (BAM) ==> counts matrix (genes as rows, samples as columns)
```

Example of a counts matrix (one row per gene):

```
gene_id          gene_name  gene_biotype            SRR2155413  SRR2155414  SRR2155415
ENSG00000160072  ATAD3B     protein_coding          114         347         317
ENSG00000279928  DDX11L17   unprocessed_pseudogene  0           0           0
ENSG00000228037             lncRNA                  0           1           1
ENSG00000142611  PRDM16     protein_coding          0           3           0
```
...
In a 'stranded' library, the reads always come from the same strand, resolving this ambiguity.
...
Use the search box at the top of the IGV viewer to find one of:
  ...
  - `RGS9BP` (`ENSG00000186326`) - Answer: <span class="redacted">(SRR2155413 = 5 reads) - the ANKRD27 gene overlaps with the RGS9BP transcript, so two (unstranded) reads are ambiguous and not counted. There is one read that doesn't quite overlap the gene.</span>
...
The reference genome is provided as a set of nucleotide sequences, usually as (gzip compressed) FASTA format - files with extensions like `.fasta.gz`, `.fa.gz` or `.fna.gz`, and generally used with the corresponding annotations (eg GTF, GFF3).
...
- this accession is the unambigious identifier that should be used in publications.
...
eg, The _nucleotide sequence_ of the Ensembl human genome release `GRCh38.p14` is equivalent to the NCBI GenBank `GCA_000001405.29` release.
...
This allows reads from repeat regions to align there rather than somewhere they shouldn't.
...
the *annotations* can vary (sometimes considerably). This is where each organization gets to be creative with their own methodology for curating manual and computational 
gene annotations.

> Tip: The structure of Ensembl annotation files tend to be more consistent (uniform use of gene_id, gene_name attributes etc), especially for non-model organisms.
